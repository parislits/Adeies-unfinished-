...
public class KanonikesFrame extends JFrame {
	...
	public KanonikesFrame(List<Worker> result, int index) {
		jframe = new JFrame();
		...
		JLabel label = new JLabel("\u039F\u03B9 \u03BA\u03B1\u03BD\u03BF\u03BD\u03B9\u03BA\u03AD\u03C2 \u03C4\u03BF\u03C5");
		...
		DefaultListModel<Kanonikes> K = new DefaultListModel<>();
		JList list = new JList(K);
		scrollPane.setViewportView(list);
		
		for(int j=0;j<result.get(0).getK().size();j++) {
			K.addElement(result.get(0).getK().get(j));
		}
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (list.getSelectedIndex() != -1) { 
					data =  (Kanonikes) list.getSelectedValue();
					System.out.println(data);
					result.get(0).getK().remove(data);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for(int i=0 ; i<result.get(0).getAl().size();i++) {
					...
					if(Integer.toString(result.get(0).getK().get(i).getEtos()).equals(search) ) {
							K.addElement(result.get(0).getK().get(i));
