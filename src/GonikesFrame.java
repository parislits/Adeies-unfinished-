...
public class GonikesFrame extends JFrame {
	...
	public GonikesFrame(List<Worker> result, int index) {
		jframe = new JFrame();
		...
		JLabel label = new JLabel("\u039F\u03B9 \u0393\u03BF\u03BD\u03B9\u03BA\u03AD\u03C2 \u03C4\u03BF\u03C5");
		...
		DefaultListModel<Gonikes> G = new DefaultListModel<>();
		JList list = new JList(G);
		scrollPane.setViewportView(list);
		
		for(int j=0;j<result.get(0).getG().size();j++) {
			G.addElement(result.get(0).getG().get(j));
		}
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (list.getSelectedIndex() != -1) { 
					data =  (Gonikes) list.getSelectedValue();
					System.out.println(data);
					result.get(0).getG().remove(data);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for(int i=0 ; i<result.get(0).getG().size();i++) {
					
					if(Integer.toString(result.get(0).getG().get(i).getEtos()).equals(search) ) {
							G.addElement(result.get(0).getG().get(i));
